...
BLEServer *pServer = nullptr; // BLE 서버 포인터 선언
bool deviceConnected = false; // BLE 장치 연결 상태 변수
unsigned long lastConnectionTime = 0; // 마지막 연결 시간 기록
const unsigned long connectionDuration = 10000; // 최대 연결 유지 시간(10초)
...
class MyServerCallbacks : public BLEServerCallbacks {
    ...
    void onDisconnect(BLEServer* pServer) {
      ...
      pServer->startAdvertising();  // 연결이 끊기면 다시 광고 시작
    }
};
...
void setup() {
  ...
  pServer->setCallbacks(new MyServerCallbacks());  // BLE 서버 콜백 설정
...
void loop() {
  // BLE 연결 상태 확인 후 자동 연결 종료 (10초 유지)
  if (deviceConnected && (millis() - lastConnectionTime > connectionDuration)) {
    pServer->disconnect(0);  // 10초 후 BLE 연결 해제
  }
